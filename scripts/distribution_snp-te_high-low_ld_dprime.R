...
for (te in TEs.chr) {
  ...
  if (NROW(snps_LD_with_te) > 0) {
    ...
    snps_LD_with_te <- snps_LD_with_te[which(snps_LD_with_te[, "DP"] == max(snps_LD_with_te[, "DP"])), ]
    # if there are more than one SNP with the same Dprime, get the closest one to the TE
...
outfile.highest <- paste0(out.dir.ld, "/plink_results_SNPs-highest-LD-TE_Dprime_chr", chr, ".ld")
...
# first get names of SNPs that are in LD with an TE (DP>0.2)
snps.in.ld <- mclapply(1:NROW(LD_results), function(row, LD_results) {

  marker1 <- LD_results[row, "SNP_A"]
  markeDP <- LD_results[row, "SNP_B"]
  DP <- as.numeric(LD_results[row, "DP"])
...
# write filtered ld table
outfile.lowest <- paste0(out.dir.ld, "/plink_results_SNPs-lowest-LD-TE_Dprime_chr", chr, ".ld")
